...
func NewDefaultGeneratorPrefix(dir, prefix string, cfg *config.Config, opt ...Option) (*defaultGenerator, error) {
	if dir == "" {
		dir = pwd
	}
	dirAbs, err := filepath.Abs(dir)
...
func (g *defaultGenerator) StartFromInformationSchema(tables map[string]*model.Table, withCache, strict bool) error {
	...
	for _, each := range tables {
		...
		//trim prefix
		if len(g.prefix) > 0 {
			table.Name = table.Name.Prefix(g.prefix)
		}
