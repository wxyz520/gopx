...
import (
	...
	"gopx/config"
	"gopx/model/sql/command/migrationnotes"
	"gopx/model/sql/gen"
	"gopx/model/sql/model"
	"gopx/model/sql/util"
	file "gopx/util"
	"gopx/util/console"
	"gopx/util/pathx"
...
func PostgreSqlDataSource(_ *cobra.Command, _ []string) error {
	...
	return fromPostgreSqlDataSource(url, pattern, dir, schema, prefix, cfg, cache, idea, VarBoolStrict, ignoreColumns)
}
...
func fromMysqlDataSource(arg dataSourceArg) error {
	...
	generator, err := gen.NewDefaultGeneratorPrefix(arg.dir, arg.prefix, arg.cfg,
		gen.WithConsoleOption(log), gen.WithIgnoreColumns(arg.ignoreColumns))
	if err != nil {
		return err
	}
...
func fromPostgreSqlDataSource(url, pattern, dir, schema, prefix string, cfg *config.Config, cache, idea, strict bool, ignoreColumns []string) error {
